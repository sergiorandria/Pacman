    ...
    class Ghost : public g_PacmanEntityDecl::AliveEntity, public sf::Drawable {
        public:
            ...
            std::uint16_t getGridX() const { 
                return static_cast<std::uint16_t>(sprite_->getPosition().x) / blockSize; 
            }

            std::uint16_t getGridY() const { 
                return static_cast<std::uint16_t>(sprite_->getPosition().y) / blockSize; 
            }

            bool isScared() const { return isScared_; }

            void respawn() {
                // Reset to spawn position (will be set by Game)
                isScared_ = false;
                // Position will be reset by Game::respawnGhosts()
            }

            // Store spawn position
            void setSpawnPosition(std::uint16_t gridX, std::uint16_t gridY) {
                spawnX_ = gridX;
                spawnY_ = gridY;
            }
        ...
        private:
            ...
            std::uint16_t spawnX_;
            std::uint16_t spawnY_;
